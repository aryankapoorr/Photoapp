  ...
  const handleNameSubmit = async () => {
    try {
      if (currentUserId && name) {
        ...
        await updateDoc(userRef, { name, photos: [] });
    ...
    try {
      ...
      if (error.code === 'auth/email-already-in-use') {
        try {
          ...
          if (!userDoc.exists()) {
            await setDoc(userRef, { email: user.email, photos: [] });
    ...
    <GoogleOAuthProvider clientId={process.env.REACT_APP_GOOGLE_CLIENT_ID}>
      <Box className="login-container">
        ...
        <Box className="graphic-container">
          <img src={graphic} alt="Login Graphic" className="login-graphic" />
        </Box>
        ...
        <Card className="login-card">
          ...
          <Divider className="divider-or">OR</Divider>
